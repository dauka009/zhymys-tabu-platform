...
export default async function VacanciesPage() {
  // Серверден (PostgreSQL) вакансияларды тарту
  const vacanciesData = await getAllVacanciesFromDb();
    ...
    <main className="flex-1 bg-muted/20">
      <div className="bg-[#0F172A] py-12 text-white shadow-inner relative overflow-hidden">
        ...
        <div className="container px-4 relative z-10">
          ...
          <p className="mt-2 text-indigo-200">Сізге сәйкес келетін арман жұмысын табыңыз (Базадан тартылды)</p>
        </div>
      </div>
      ...
      <div className="container px-4 py-10">
        <div className="flex flex-col lg:flex-row gap-8 items-start">
          ...
          <div className="w-full lg:w-2/3 xl:w-3/4">
            <VacancyList initialData={vacanciesData} />
          </div>
        </div>
      </div>
    </main>
